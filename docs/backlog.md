...
### Gateway 403 — RESOLVED (2026-08-12): unapproved Tailscale subnet route

While deploying KubeOpenCode (separate PR), its HTTPRoute hit the same `403 Access denied` documented above, cluster-wide (every app behind `homelab-gateway`, including grafana). Ran the actual documented diagnostic (Envoy debug logging + NPDS dump — see `Agent.md`) and confirmed it wasn't Tailscale-specific at the packet level (reproduced with a pure in-cluster pod → Gateway-IP request) and wasn't caused by the kubeopencode work itself.

Actual fix: the `192.168.178.200/32` subnet route had lost its approval in the Tailscale admin console. Re-approving it resolved access immediately. Plausible unifying explanation for the in-cluster reproduction too: the subnet router and Cilium's L2Announce both claim that IP on the same host, so an unapproved route could disturb local routing enough to produce the same symptom even for traffic that never touched Tailscale — not independently confirmed at the packet level. Full detail in `Agent.md`'s "RESOLVED (2026-08-12)" section. If this resurfaces, check Tailscale route approval before re-running the Envoy/NPDS diagnostic.
